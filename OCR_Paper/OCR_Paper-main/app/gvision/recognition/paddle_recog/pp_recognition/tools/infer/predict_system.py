...
def sorted_boxes_custom(list_merged_box):
    """
    TODO thainq:
    Sort text boxes in order from top to bottom, left to right
    """
    list_sorted_box = sorted(list_merged_box, key=lambda item: item['box'][1], reverse=False)
    num_boxes = len(list_sorted_box)

    for i in range(num_boxes - 1):
        for j in range(i, 0, -1):
            if abs(list_sorted_box[j + 1]['box'][1] - list_sorted_box[j]['box'][1]) < 10 and \
                    (list_sorted_box[j + 1]['box'][0] < list_sorted_box[j]['box'][0]):
                tmp = list_sorted_box[j]
                list_sorted_box[j] = list_sorted_box[j + 1]
                list_sorted_box[j + 1] = tmp
            else:
                break
    return list_sorted_box
